        ...
        content.innerHTML = `
            <div class="data-table-wrapper">
                <table class="data-table">
                    <thead>
                        <tr>
                            <th data-sort="display_name" class="${this.state.sort_by === 'display_name' ? 'sorted' : ''}">Name${sortIcon('display_name')}</th>
                            <th data-sort="version" class="${this.state.sort_by === 'version' ? 'sorted' : ''}">Version${sortIcon('version')}</th>
                            <th data-sort="manufacturer" class="${this.state.sort_by === 'manufacturer' ? 'sorted' : ''}">Manufacturer${sortIcon('manufacturer')}</th>
                            <th data-sort="year" class="${this.state.sort_by === 'year' ? 'sorted' : ''}">Year${sortIcon('year')}</th>
                            <th data-sort="rating" class="${this.state.sort_by === 'rating' ? 'sorted' : ''}">Rating${sortIcon('rating')}</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${this.state.tables.map(t => `
                            <tr data-id="${t.id}" style="cursor: pointer;">
                                <td style="font-weight: 600; color: var(--text-primary);">${this.escHtml(t.display_name)}</td>
                                 <td>
                                    <div style="display: flex; align-items: center; gap: 6px;">
                                        ${(() => {
                                            const hasDirect = t.latest_vps_version && t.version && t.latest_vps_version !== t.version && t.latest_vps_version !== t.ignored_version;
                                            const hasCommunity = t.community_vps_version && t.version && t.community_vps_version !== t.version && t.community_vps_version !== t.ignored_version && t.is_community_newer;
                                ...
                                <td>${t.year || '—'}</td>
                                <td>${t.rating ? '★'.repeat(t.rating) : '<span style="color: var(--text-muted);">—</span>'}</td>
                                <td>
                                    <div class="btn-group">
            ...
            body.innerHTML = `
                ...
                <div style="margin-top: var(--space-md); padding-top: var(--space-md); border-top: 1px solid var(--glass-border);">
                    ...
                    <div style="font-size: 0.82rem; color: var(--text-tertiary); display: grid; gap: var(--space-xs);">
                        ...
                        <div><strong>Version:</strong> <span class="badge ${t.version ? (t.latest_vps_version && t.latest_vps_version !== t.version && t.latest_vps_version !== t.ignored_version ? 'badge-warning' : 'badge-success') : 'badge-neutral'}">${t.version || 'Unknown'}</span></div>
                        
                        ${t.latest_vps_version && t.latest_vps_version !== t.version && t.latest_vps_version !== t.ignored_version ? `
                        ...
                        ${t.is_community_newer && t.community_vps_version !== t.version && t.community_vps_version !== t.ignored_version ? `
