...
class GamelistManager:
    ...
    def update_game(self, rom_path: str, metadata: Dict[str, str]):
        ...
        for game in root.findall("game"):
            ...
            if path_elem is not None and self._normalize_path(path_elem.text) == norm_rom_path:
                game_elem = game
                break
        ...
        if game_elem is None:
            ...
            path_elem.text = rom_path if rom_path.startswith("./") else f"./{rom_path.lstrip('./')}"
    ...
    def get_game(self, rom_path: str) -> Optional[Dict[str, str]]:
        ...
        for game in root.findall("game"):
            path_elem = game.find("path")
            if path_elem is not None and self._normalize_path(path_elem.text) == norm_path:
                res = {}
                for child in game:
    ...
    def rename_game(self, old_rom_path: str, new_rom_path: str):
        ...
        for game in root.findall("game"):
            path_elem = game.find("path")
            if path_elem is not None and self._normalize_path(path_elem.text) == norm_old:
                path_elem.text = new_rom_path if new_rom_path.startswith("./") else f"./{new_rom_path.lstrip('./')}"
                found = True
        ...
        if "/" in old_rel and "/" in new_rel:
            ...
            for fol in root.findall("folder"):
                ...
                if fpath_elem is not None and self._normalize_path(fpath_elem.text) == norm_old_folder:
                    fpath_elem.text = new_folder if new_folder.startswith("./") else f"./{new_folder.lstrip('./')}"
                    found = True
