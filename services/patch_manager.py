...
class PatchManager:
    ...
    async def get_patch_status() -> list[dict]:
        ...
        for t in tables:
            ...
            if is_extracted and not vbs_hash:
                vbs_hash = await asyncio.to_thread(vbs_manager.calculate_vbs_hash, vbs_sidecar)
                if vbs_hash:
                    # Update DB in background (don't wait for it to return result)
                    asyncio.create_task(db.upsert_table({
                        "id": t["id"],
                        "vbs_hash": vbs_hash
                    }))

            # Use unified status logic
            vbs_status = vbs_manager.resolve_vbs_status(is_extracted, vbs_hash, patch_originals, patch_final)
            ...
            results.append({
                ...
                "patch_name": patch_info.get("name", t["display_name"]) if patch_info else t["display_name"],
                "vps_id": t.get("vps_id", "")
            })
    ...
    async def apply_patch(table_id: int) -> dict:
        ...
        if not vbs_hash:
            return {"success": False, "error": "No VBS hash found for this table. Try re-scanning."}
        ...
        if not patch_info:
            return {"success": False, "error": "No patch available for this VBScript version."}
