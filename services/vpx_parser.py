...
class VPXParser:
        ...
        try:
            with olefile.OleFileIO(str(vpx_path)) as ole:
                ...
                if ole.exists('TableInfo/TableVersion'):
                    metadata["version"] = VPXParser._read_stream(ole, 'TableInfo/TableVersion')
                if ole.exists('TableInfo/AuthorName'):
                    metadata["author"] = VPXParser._read_stream(ole, 'TableInfo/AuthorName')
                if ole.exists('TableInfo/TableName'):
                    metadata["display_name"] = VPXParser._read_stream(ole, 'TableInfo/TableName')
    ...
    @staticmethod
    def process_vpx_table(vpx_filepath: str | Path, extract_sidecar: bool = True, vps_id: str = None) -> list[str]:
        """
        ...
        if vps_id:
            ...
            if vps_roms:
                logger.info(f"Using VPS-matched ROMs for table (ID: {vps_id}): {vps_roms}")
        ...
        if vbs_sidecar.exists():
            try:
                ...
                with open(vbs_sidecar, 'r', encoding='utf-8', errors='replace') as f:
                    script_text = f.read()
                logger.info(f"Using robustly extracted sidecar for ROM detection: {vbs_sidecar}")
            except Exception as e:
                    ...
                    for candidate in ['GameData/CODE', 'GameStg/GameItem']:
                        if ole.exists(candidate):
                            if candidate != 'GameData/CODE':
                                ...
                                logger.info(f"Found script in standard stream: {candidate}")
                                break
                        ...
                        for stream_parts in ole.listdir():
                            try:
                                if ole.get_type(stream_parts) == olefile.STGTY_STREAM:
                                    ...
                                    if b"cGameName" in content or b"GameName" in content or b"ROM" in content:
                                        raw_bytes = content
                                        break
                            except: continue
    ...
    def detect_rom(vpx_path: str | Path, vps_id: str = None) -> str:
        ...
        roms = VPXParser.process_vpx_table(vpx_path, extract_sidecar=False, vps_id=vps_id)
