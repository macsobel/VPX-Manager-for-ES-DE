    ...
    async def check_for_updates():
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if not latest_tag:
                    return {"update_available": False, "error": "No version found on GitHub"}
                ...
                return {
                    ...
                    "download_url": release_data.get("assets", [{}])[0].get("browser_download_url") if release_data.get("assets") else release_data.get("html_url"),
                    "published_at": release_data.get("published_at"),
                    "body": release_data.get("body", "")
                }
        except httpx.HTTPStatusError as e:
            logger.error(f"GitHub API error: {e}")
            return {"update_available": False, "error": f"GitHub API error: {e.response.status_code}"}
