...
HASHES_URL = "https://raw.githubusercontent.com/jsm174/vpx-standalone-scripts/master/hashes.json"
...
class VBSManagerService:
    ...
    async def get_patch_hashes(self, force_refresh: bool = False) -> list[dict]:
        ...
        if not force_refresh and self._cached_hashes is not None and local_path.exists():
            return self._cached_hashes
        ...
        if not force_refresh and local_path.exists():
            ...
            if (time.time() - mtime) < ttl_seconds:
                try:
                    self._cached_hashes = await asyncio.to_thread(self._load_hashes_sync, local_path)
                    return self._cached_hashes
    ...
    async def _download_and_apply_patch(self, patch_url: str, vbs_target: Path) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                await asyncio.to_thread(self._write_vbs_sync, vbs_target, response.content)
    ...
    async def extract_vbs(self, vpx_path: Path) -> dict:
        ...
        allowed_binaries = {"VPinballX_GL", "VPinballX_BGFX", "VPinballX", "vpinballx", "vpinballx-gl"}
        if executable.name not in allowed_binaries:
            logger.error(f"[v2.2.diag] Security violation: Executable name '{executable.name}' is not allowed.")
            return {"success": False, "error": f"Security violation: Executable '{executable.name}' is not an expected VPX binary."}
        ...
        try:
            ...
            try:
                ...
                process.kill()
                await process.wait()
                logger.error(f"[v2.2.diag] Extraction TIMEOUT after 30s for: {vpx_path}")
                return {"success": False, "error": f"Extraction timed out after 30 seconds."}

            if process.returncode != 0:
                 error_msg = stderr.decode().strip() if stderr else "No error output."
                 logger.error(f"[v2.2.diag] Extraction failed with code {process.returncode}. Stderr: {error_msg}")
                 return {"success": False, "error": f"Extraction Failed [v2.2.diag]. Stderr: {error_msg}. Please ensure your VPX application is cleared from macOS quarantine using: xattr -rc '{config.vpx_standalone_app_path}'"}
            ...
            if vbs_path.exists():
                ...
                if vbs_hash:
                    ...
                    if table:
                        logger.info(f"[v2.2.diag] Updating DB for {vpx_path.name} with hash {vbs_hash}")
                ...
                logger.error(f"[v2.2.diag] Command finished but {vbs_path} does not exist.")
                return {"success": False, "error": f"Extraction command completed (code {process.returncode}) but .vbs file was not found. Stderr: {stderr.decode().strip() if stderr else 'None'}"}
        except PermissionError as e:
             logger.error(f"[v2.2.diag] PermissionError: {e}")
             return {"success": False, "error": f"Permission denied [v2.2.diag] (PermissionError: {e}). Please ensure your VPX application is cleared from macOS quarantine and is executable: xattr -rc '{config.vpx_standalone_app_path}'"}
        except Exception as e:
             logger.exception(f"[v2.2.diag] Unexpected error during extraction")
             return {"success": False, "error": f"An unexpected error occurred [v2.2.diag]: {str(e)}"}
    ...
    async def apply_verified_patch(self, table_id: int) -> dict:
        ...
        if not vbs_path.exists():
            return {"success": False, "error": "VBS sidecar missing. Please extract first."}
        ...
        patch_entry = next((item for item in hashes if item["sha256"] == current_hash), None)
        
        if not patch_entry:
            return {"success": False, "error": "No matching original script found in the patch database. Your VBS may already be modified or unknown."}

        patch_url = patch_entry.get("patched", {}).get("url")
        if not patch_url:
            return {"success": False, "error": "Patch URL not found in the hash database."}
        ...
        if result["success"]:
            ...
            return {"success": True, "message": "Verified patch applied and hash updated."}
        
        return result

    def resolve_vbs_status(self, is_extracted: bool, vbs_hash: str, patch_originals: dict, patch_final: dict) -> str:
    ...
    async def _bulk_extract_worker(self, extraction_list, indexing_list):
        ...
        try:
            ...
            task_registry.start_task(task_id, total=total, message="Synchronizing scripts...")

            # 1. Perform extractions (and hashes)
            for vpx_path in extraction_list:
                try:
                    task_registry.update_progress(task_id, self._bulk_extract_progress["current"], f"Extracting {vpx_path.name}...")
            ...
            for vpx_path, vbs_path in indexing_list:
                try:
                    task_registry.update_progress(task_id, self._bulk_extract_progress["current"], f"Hashing {vpx_path.name}...")
                    vbs_hash = await asyncio.to_thread(self.calculate_vbs_hash, vbs_path)
                    if vbs_hash:
                        await db.upsert_table({
                            "filename": vpx_path.name,
    ...
    async def start_bulk_extraction(self):
        ...
        self._bulk_extract_task = asyncio.create_task(self._bulk_extract_worker(extraction_list, indexing_list))
        return {"success": True, "message": f"Bulk extraction started for {total_work} tables."}
    ...
    def apply_regex_fix(self, vbs_content: str, fix_type: str, rom_name: Optional[str] = None) -> str:
        if fix_type == "colordmd":
            ...
            pattern = re.compile(r'(?i)\b(?:cGameName|GameName|RomName|ROM)\s*=\s*"([^"]+)"')
