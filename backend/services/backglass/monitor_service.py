...
class BackglassMonitor:
    ...
    def start_companion(self):
        ...
        try:
            ...
            # macOS: Return focus to ES-DE after a brief delay to allow the window to open
            def return_focus():
                time.sleep(1.0)
                try:
                    # Try to activate ES-DE to return focus to the frontend
                    subprocess.run(["osascript", "-e", 'activate application "ES-DE"'], 
