...
def test_normalize_video_command_injection(mock_replace, mock_run, mock_check_call):
    # Mock ffmpeg run output to trigger ffmpeg transcode call
    mock_run.return_value.stderr = "Stream #0:0(eng): Video: h264 (Main) (avc1 / ...), yuv444p, 1920x1080"

    malicious_filename = "-test.mp4"
    normalize_video(malicious_filename)

    # Assert ffmpeg check_output is secure
    assert mock_run.called
    cmd_meta = mock_run.call_args[0][0]
