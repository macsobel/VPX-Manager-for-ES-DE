...
async def _run_bulk_generate(tables: list):
    ...
    task_registry.start_task("ini-bulk", total=len(tables), message="Generating missing INIs...")
    ...
    try:
        for t in tables:
            ...
            task_registry.update_progress("ini-bulk", completed, f"Generating: {t['display_name']}")
        ...
        task_registry.complete_task("ini-bulk", f"Completed {completed} INIs with {errors} errors")
...
async def bulk_generate_ini():
    ...
    if not to_process:
        return {"success": True, "message": "All tables already have INI files", "count": 0}
        
    asyncio.create_task(_run_bulk_generate(to_process))
    return {"success": True, "message": f"Started generating {len(to_process)} INI files", "count": len(to_process)}
