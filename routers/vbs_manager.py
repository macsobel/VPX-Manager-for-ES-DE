...
async def get_vbs_status(background_tasks: BackgroundTasks):
    ...
    patch_final = {p["patched"]["sha256"]: p for p in patches if "patched" in p and "sha256" in p["patched"]}
        ...
        if is_extracted and not vbs_hash:
            try:
                ...
                if vbs_hash:
                    ...
                    background_tasks.add_task(db.upsert_table, {"id": t["id"], "vbs_hash": vbs_hash})
            except Exception as e:
                logger.error(f"Failed to auto-hash in status check: {e}")

        status = vbs_manager.resolve_vbs_status(is_extracted, vbs_hash, patch_originals, patch_final)
...
async def get_table_vbs_details(table_id: int):
    ...
    if is_extracted and not vbs_hash:
        try:
            ...
            if vbs_hash:
                ...
                logger.info(f"Updated missing VBS hash for table {table_id}: {vbs_hash}")
        except Exception as e:
    ...
    patch_final = {p["patched"]["sha256"]: p for p in hashes if "patched" in p and "sha256" in p["patched"]}
...
async def reset_table_vbs(table_id: int):
    ...
    if vbs_sidecar.exists():
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to delete VBS file: {e}")
    else:
        # Even if file is missing, clear the hash if it exists in DB just in case
        await db.upsert_table({"id": table_id, "vbs_hash": ""})
        return {"success": True, "message": "No VBS sidecar found; database hash cleared"}
...
async def get_regex_diff(table_id: int, req: RegexFixRequest):
    ...
    if not vbs_sidecar.exists():
        raise HTTPException(status_code=400, detail="VBS sidecar not found. Please extract first.")

    try:
        with open(vbs_sidecar, "r", encoding="utf-8", errors="replace") as f:
            original_content = f.read()

        new_content = vbs_manager.apply_regex_fix(original_content, req.fix_type, req.rom_name)

        if original_content == new_content:
             return {"success": True, "changes_found": False, "message": "No matches found for this fix."}
