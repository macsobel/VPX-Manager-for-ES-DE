...
from services.screenscraper import (
    test_credentials,
    search_game,
    download_media_for_table,
    get_quota_info,
    MEDIA_TYPE_MAP,
)
from services.vpinmediadb import scrape_vpinmediadb
...
async def get_batch_status(table_ids: list[int]):
    ...
    from services.media_manager import get_all_esde_media_status
        ...
        for dl_item in result["downloaded"]:
            ...
            if cat and path:
                try:
                    ...
                    logger.error(f"Failed to persist media [{cat}] for table {table_id}: {db_err}")
...
async def download_all_media(missing_only: bool = True):
    ...
    if not tables_to_scrape:
        return {"success": True, "message": "No tables match the scraping criteria", "total": 0}

    task_registry.start_task("batch-scraper", total=len(tables_to_scrape), message="Starting batch scrape...")
...
async def scrape_manual_for_table(table_id: int):
    ...
    from services.screenscraper import download_media_for_table
            ...
            for dl_item in result["downloaded"]:
                ...
                if cat and path:
                    try:
                        ...
                        logger.error(f"Failed to persist manual [{cat}] for table {table_id}: {db_err}")
...
async def start_batch_manual_download():
    ...
    if task.status == "running":
        return {"success": False, "message": "A batch manual scrape is already in progress"}

    # We'll do the filtering inside the background task to avoid blocking the request
    task_registry.start_task("manual-batch-scraper", total=0, message="Initializing batch manual scrape...")
...
async def _run_batch_manual_scrape():
    ...
    try:
        ...
        task_registry.update_progress("manual-batch-scraper", 0, "Checking for missing manuals...")
        ...
        task_registry.start_task("manual-batch-scraper", total=len(tables_to_scrape), message=f"Starting download of {len(tables_to_scrape)} manuals...")
        ...
        for t in tables_to_scrape:
            ...
            task_registry.update_progress("manual-batch-scraper", completed, f"Downloading manual: {t['display_name']}")
                    ...
                    for dl_item in result.get("downloaded", []):
                        ...
                        if cat and path:
                            try:
                                ...
                                logger.error(f"Failed to persist manual [{cat}] for {t['display_name']}: {db_err}")
                    
                    await db.upsert_table({"id": t["id"], "has_manual": 1})
                
                if result.get("errors"):
                    logger.error(f"Manual download error for {t['display_name']}: {result['errors']}")
        ...
        task_registry.complete_task("manual-batch-scraper", f"Completed {completed} tables with {errors} errors")
...
async def _run_batch_scrape(tables_to_scrape: list, missing_only: bool = True):
    ...
    try:
        for item in tables_to_scrape:
            ...
            t = item["table"]
            task_registry.update_progress("batch-scraper", completed, f"Scraping: {t['display_name']}")
                    ...
                    for dl_item in result["downloaded"]:
                        ...
                        if cat and path:
                            try:
                                ...
                                logger.error(f"Failed to persist media [{cat}] for {t['display_name']}: {db_err}")
                
                # Check for errors in the payload
                if result.get("errors"):
                    logger.error(f"Media processor errors for {t['display_name']}: {result['errors']}")
            except Exception as e:
                errors += 1
        ...
        task_registry.complete_task("batch-scraper", f"Completed {completed} tables with {errors} errors")
...
async def cancel_manual_batch_scrape():
    ...
    if task.status == "running":
        ...
        return {"success": True, "message": "Batch manual scrape cancellation requested"}
