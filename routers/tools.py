...
async def apply_esde_integration():
    try:
        # 1. Generate the launcher script in a space-free path
...
# Visual Pinball Launch Script for ES-DE on macOS
...
# cd into the MacOS directory so the binary can find its relative dependencies
cd "$VP_MAC_OS_DIR" || exit 1
